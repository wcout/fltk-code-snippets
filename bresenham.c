...
{                                  /* plot a limited quadratic Bezier segment */
  ...
  double err, cur = xx*sy-yy*sx;                                 /* curvature */
  ...
  if (cur != 0) {                                         /* no straight line */
    ...
    int xy = 2*xx*yy; xx *= xx; yy *= yy;           /* differences 2nd degree */
...
{                                    /* plot an anti-aliased line of width wd */
   ...
   for (wd = (wd+1)/2; ; ) {                                    /* pixel loop */
      setPixelAA(x0, y0, std::max(0,(int)(255*(abs(err-dx+dy)/ed-wd+1))));
      e2 = err; x2 = x0;
      if (2*e2 >= -dx) {                                            /* x step */
         for (e2 += dy, y2 = y0; e2 < ed*wd && (y1 != y2 || dx > dy); e2 += dx)
            setPixelAA(x0, y2 += sy, std::max(0,(int)(255*(abs(e2)/ed-wd+1))));
         if (x0 == x1) break;
         e2 = err; err -= dy; x0 += sx;
      }
      if (2*e2 <= dy) {                                             /* y step */
         for (e2 = dx-e2; e2 < ed*wd && (x1 != x2 || dx < dy); e2 += dy)
            setPixelAA(x2 += sx, y0, std::max(0,(int)(255*(abs(e2)/ed-wd+1))));
         if (y0 == y1) break;
         err += dx; y0 += sy;
      }
   }
